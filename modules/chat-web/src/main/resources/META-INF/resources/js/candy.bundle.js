var countCstm = 0;
var countUserLoading = 0;
var countUserLoadingClick = 0;
     ...
     */
        addTab: function(roomJid, roomName, roomType) {
            ...
            var evtData = {
                ...
                roomName: roomName,
                roomType: roomType,
                roomTypeIconOptions: (roomType == 'groupchat' ? '<span class="glyphicon glyphicon-option-vertical" aria-hidden="true"></span>' : 'test'),
                roomId: roomId
            };
   ...
   */
     ...
     */
        submit: function(event) {
            // $("#chat-autoscroll-control").trigger('click');
            // alert('Yes123');
     ...
     */
       ...
       */
            ...
            var cur_user = $('#cstmOnlineUsers .online-users-cstm.user.me .label h5').last().html().replace('<span><i class="glyphicon glyphicon-user"></i></span> ', '');
            var renderEvtData = {
                ...
                templateData: {
                    ...
                    roomjid: roomJid,
                    from: from,
                    // messageReceivedClass : ($('#userScreenName').val() != name ? 'bubble_response' : ''),
                    messageReceivedClass2 : (cur_user != name ? 'pull-left' : 'pull-right')
                },
                stanza: stanza
            };
   ...
   */
       ...
       */
            ...
            if (action === "join") {
                ...
                if (self.Chat.rooms[roomJid].type === "chat") {
                    ...
                    // console.log("user.data['jid']", user.data['jid']);
                    $("#chat-tabs div[data-roomjid='" + user.data['jid'] + "']").find('div.dot').hide();
                } else {
     ...
     */
        userClick: function() {
            ...
            if(countUserLoadingClick == 0) {
                var switchToRoomClick = false;
            } else {
                var switchToRoomClick = true;
            }
            self.PrivateRoom.open(targetJid, elem.attr("data-nick"), switchToRoomClick, useRealJid);
        },
            ...
            if (!user.getPreviousNick() || !$rosterUserElem || $rosterUserElem.is(":visible") === false) {
                ...
                if (currentUser !== undefined && user.getNick() !== currentUser.getNick() && self.Room.getUser(roomJid)) {
                    ...
                    if (self.Chat.rooms[roomJid].type === "chat") {
                        ...
                        $("#chat-tabs div[data-roomjid='" + user.data['jid'] + "']").find('div.dot').show()
                    } else {
...
Candy.View.Template = function(self) {
    ...
    self.Chat = {
        ...
        rooms: '<div class="col-md-8 vertical-border">'+
        			'<div class="main-chat">'+
        				'<div id="chat-rooms" class="rooms"></div>'+
        			'</div>'+
        	   '</div>',
        tabs: '<div class="col-md-4 white-background"><div class="row"><div class="user-list col-md-12"><div class="col-md-10"><h4>Groups</h4></div><div class="col-md-2 plus"><span class="addGroupModal"><i class="glyphicon glyphicon-plus"></i></span></div></div></div>'+
                    '<div id="chat-tabs" class="lists"></div>'+
        			'<div class="roster-pane"></div>'+
        	  '</div>',
        mobileIcon: '<div id="mobile-roster-icon"><a class="box-shadow-icon"></a></div>',
        tab: '<div class="users-list clearfix roomtype-{{roomType}}"  data-roomjid="{{roomJid}}" data-roomtype="{{roomType}}">'+
                    '<div class="col-md-2">'+
                        '<img class="chatImg" src="/o/chat.web/images/{{roomType}}.png" alt="{{roomType}}" />'+
                    '</div>'+
                    '<div class="col-md-10">'+
                        '<h5 class="chat-title-h5">{{#privateUserChat}}@{{/privateUserChat}}{{#groupChat}}##{{/groupChat}}{{name}}</h5>'+
                        '<span class="glyphicon glyphicon-remove-circle cstm-cr-ver-menu deleteGroupModal" data-id="{{#privateUserChat}}@{{/privateUserChat}}{{#groupChat}}##{{/groupChat}}{{name}}" title="Delete group" aria-hidden="true"></span>'+
                    '</div>'+
                    '<div class="dot"></div>'+
                '</div>'+

                '<hr class="userlistline" />',
        user: '<div class="user role-{{role}} affiliation-{{affiliation}}{{#me}} me{{/me}}"' + ' id="user-{{roomId}}-{{userId}}" data-jid="{{userJid}}" data-real-jid="{{realJid}}"' + ' data-nick="{{nick}}" data-role="{{role}}" data-affiliation="{{affiliation}}" data-status="{{status}}">' + '<div class="users-list clearfix"><div class="label"><h5 class="text-left"><span><i class="glyphicon glyphicon-user"></i></span> {{displayNick}}</h5></div></div><ul>' + '<li class="context" id="context-{{roomId}}-{{userId}}">&#x25BE;</li>' + '<li class="role role-{{role}} affiliation-{{affiliation}}" data-tooltip="{{tooltipRole}}"></li>' + '<li class="ignore" data-tooltip="{{tooltipIgnored}}"></li></ul></div>',
    ...
    self.Roster = {
        ...
        user: '<div class="online-users-cstm user role-{{role}} affiliation-{{affiliation}}{{#me}} me{{/me}}"' + ' id="user-{{roomId}}-{{userId}}" data-jid="{{userJid}}" data-real-jid="{{realJid}}"' + ' data-nick="{{nick}}" data-role="{{role}}" data-affiliation="{{affiliation}}" data-status="{{status}}">' + '<div class="users-list clearfix"><div class="label"><h5 class="text-left"><span><i class="glyphicon glyphicon-user"></i></span> {{displayNick}}</h5></div></div><ul>' + '<li class="context" id="context-{{roomId}}-{{userId}}">&#x25BE;</li>' + '<li class="role role-{{role}} affiliation-{{affiliation}}" data-tooltip="{{tooltipRole}}"></li>' + '<li class="ignore" data-tooltip="{{tooltipIgnored}}"></li></ul></div>'
    };
    self.Message = {
        pane: '<div class="message-pane-wrapper"><ul class="message-pane"></ul></div>',
        item: '<li><div class="user-img {{messageReceivedClass2}}"><div class="msg-body"><div class="message left-user">{{{message}}}</div><div class="time">{{time}}</div> </div><div class="user-imgin"><img src="/o/chat.web/images/chat.png" alt="{{displayName}}" class="chat-img" title="{{displayName}}"><strong>{{displayName}}</strong></div></div></li>'
    };
    self.Login = {
...
function scrollTopAnimate() {
    var elem = $('.message-pane');
    elem.animate({
        scrollTop: $(".message-pane")[$("#chat-tabs div.active").index() / 2].scrollHeight
    });
}

// Online Users List Ends
function singleUserShow(){
    $('#cstmOnlineUsers .online-users-cstm.user:not(#cstmOnlineUsers .online-users-cstm.user.me)').each(function() {
        var elemCSTM = $(this);
        var realJidCSTM = elemCSTM.attr("data-real-jid");
        var dataNickCSTM = elemCSTM.attr("data-nick");
        var useRealJidCSTM = Candy.Core.getOptions().useParticipantRealJid && (realJidCSTM !== undefined && realJidCSTM !== null && realJidCSTM !== "");
        var targetJidCSTM = useRealJidCSTM && realJidCSTM ? Strophe.getBareJidFromJid(realJidCSTM) : elemCSTM.attr("data-jid");
        Candy.View.Pane.PrivateRoom.open(targetJidCSTM, elemCSTM.attr("data-nick"), false, useRealJidCSTM);
    });
    $('.room-pane.roomtype-chat').hide()
}

setTimeout(singleUserShow, 1000);
// Custom JS Ends